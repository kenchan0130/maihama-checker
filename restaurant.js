...
const loopLimitMs = 1000 * 60 * 45; // 45 min
...
(async () => {
  ...
  while (true) {
    ...
    if (counter > 0) {
      console.log(
        `reserve page waiting ${loopWaitMs} ms, loop ${loopTime} ms...`
      );
    ...
    while (page.url() !== url) {
      ...
      console.log({
        currentUrl: page.url(),
        waitTime: dom.window.document.querySelector("#MainPart_lbWhichIsIn")
          .textContent,
        accessTime: dom.window.document.querySelector(
          "#MainPart_lbExpectedServiceTime"
        ).textContent,
        updateTime: dom.window.document.querySelector(
          "#MainPart_lbLastUpdateTimeText"
        ).textContent,
      });
    ...
    const reserveSiteCookies = cookies.filter(
      (v) => v.name.match(/^JSESSIONID$/) || v.name.match(/^QueueITAccepted/)
    );
    ...
    const source = await page.content();
    const dom = new JSDOM(source);
    const hasGotReservationDom =
      dom.window.document.querySelectorAll(".hasGotReservation");
